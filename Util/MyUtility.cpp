...
std::string SpaceWar::MyUtility::getCurrentTime()
{
	time_t t;
	time(&t);
	char tmp[64];
...
std::string SpaceWar::MyUtility::gbk_2_utf8(const string text)
{
...
std::string SpaceWar::MyUtility::getUTF8Char(const string key)
{
	auto sharedFileUtils = FileUtils::getInstance();

	std::string fullPathForFilename = sharedFileUtils->fullPathForFilename(GameTexture::getInstance()->getTextureRelativePath("utf8_char.plist"));
