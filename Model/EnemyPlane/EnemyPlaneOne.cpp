...
void SpaceWar::EnemyPlaneOne::update(float dt)
{
	EnemyPlane::update(dt);
}

void SpaceWar::EnemyPlaneOne::onEnter()
{
	EnemyPlane::onEnter();
}

void SpaceWar::EnemyPlaneOne::onExit()
{
	//取消订阅的信息..
...
void SpaceWar::EnemyPlaneOne::cleanup()
{
	EnemyPlane::cleanup();

}

void SpaceWar::EnemyPlaneOne::activate()
{
	...
	schedule(CC_CALLBACK_0(EnemyPlaneOne::move, this),
		0.1,
		CC_REPEAT_FOREVER,
		0,
		string("EnemyPlaneOne::move"));


}

void SpaceWar::EnemyPlaneOne::shoot()
{
...
//
void SpaceWar::EnemyPlaneOne::move()
{
...
void SpaceWar::EnemyPlaneOne::manualDestroy(float time)
{
	//
...
SpaceWar::EnemyPlaneOne* SpaceWar::EnemyPlaneOne::clone()
{
	EnemyPlaneOne* plane = EnemyPlaneOne::create(this->m_filename);
...
void SpaceWar::EnemyPlaneOne::BeginContact(cocos2d::Vec2 contact, CPhysicsSprite* other)
{
...
void SpaceWar::EnemyPlaneOne::EndContact(cocos2d::Vec2 contact, CPhysicsSprite* other)
{
...
//
bool SpaceWar::EnemyPlaneOne::init()
{
	bool bRet = false;
	do
	{
		...
		this->setSprayingLeftPoint(Vec2(8,27));
		this->setSprayingRightPoint(Vec2(78,27));
		this->setShootingPoint(Vec2(42,64));
		this->setMiddlePoint(Vec2(43,34));
		this->setShootingLeftPoint(Vec2(31,64));
		this->setShootingRightPoint(Vec2(55,64));
		this->setHp(50);
		...
		schedule(CC_CALLBACK_0(EnemyPlaneOne::shoot, this),
			0.1f,
			CC_REPEAT_FOREVER,
			0,
			"EnemyPlaneOne::shoot");
...
void SpaceWar::EnemyPlaneOne::speak()
{
	auto pos = this->getPosition();
	log("EnemyPlaneOne:%s:Position:(%f,%f)", this->getGuid().c_str(), pos.x, pos.y);

}

SpaceWar::EnemyPlaneOne::~EnemyPlaneOne()
{
...
SpaceWar::EnemyPlaneOne* SpaceWar::EnemyPlaneOne::create(const std::string& filename)
{
	EnemyPlaneOne *sprite = new (std::nothrow) EnemyPlaneOne();
